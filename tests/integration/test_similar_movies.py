...
def test_top_3_boosted_tags():
    reference_movies = [movie_ids.LOST_IN_TRANSLATION]
    results = similar_movies.get_similar(reference_movies, number_of_results=3, tags_to_boost=['fantasy'])

    assert movie_ids.THERE_WILL_BE_BLOOD_SIMILAR_MOVIES == set(results.get(movie_ids.LOST_IN_TRANSLATION).keys())
